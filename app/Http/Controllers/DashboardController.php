...
class DashboardController extends Controller
{
    public function dashboard()
    {
        ...
        $data_barang_masuk = DB::table(DB::raw('(SELECT 1 AS bulan UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7 UNION SELECT 8 UNION SELECT 9 UNION SELECT 10 UNION SELECT 11 UNION SELECT 12) as bulan'))
            ->leftJoin(DB::raw(sprintf('(SELECT COALESCE(SUM(transaksi_barangs.quantity), 0) AS jumlah_total_barang, MONTH(transaksis.tanggal) AS bulan_tanggal
                FROM transaksis
                LEFT JOIN transaksi_barangs ON transaksis.id_transaksi = transaksi_barangs.id_transaksi
                WHERE 
